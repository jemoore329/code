...
API URL example
url = "https://www.shopadvisor.com/api/ph/product/e3d147b2f60f8397f73c396d809b608f68a08e21/20131101/20140115"
...
def query_ph(startdate, enddate, productid, trace=False):
    ...
    url = baseurl + productid + "/" + startdate + "/" + enddate + "?filter=mean"
    if trace: print url
    results = dict()
    try:
        ...
        for v in data:
            date = datetime.datetime.fromtimestamp(int(v[0])).strftime('%Y-%m-%d')
            if results[productid].has_key(date):
                results[productid][date] = v[1] 
    except ValueError as e:
        if trace: print "No data",e
...
for i in content:
    ...
    print_comma_separated_set(startdate, enddate, query_ph(startdate,enddate,i.strip()))
